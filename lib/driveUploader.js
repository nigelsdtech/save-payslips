...
async function getPayslipFolderInfo ({folderName} = {}) {
  ...
  const freetextSearch = `name='${folderName}' and mimeType ='application/vnd.google-apps.folder'`;
...
async function getKnownPayslips () {
  ...
  const freetextSearch = `'${payslipFolderId}' in parents`;
  ...
  .then((payslips) => {
    return payslips.map((payslip) => {
      const [yyyy, mm, dd] = payslip.name.split('-')
