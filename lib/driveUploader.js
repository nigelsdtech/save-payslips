...
function UploadPayslip(params,callback) {
  ...
  var freetextSearch = "fullText contains '" + cfg.drive.payslipsFolderName + "' and mimeType = 'application/vnd.google-apps.folder'";
  ...
  g.listFiles({
    ...
    log.info('drive: Got folder: %s (%s)', results[0].title, results[0].id);
    var parentId = results[0].id,
        parentUrl = results[0].alternateLink;
    ...
    g.createFile({
      ...
      log.info('drive: Payslip uploaded to ' + resp.webContentLink)
      callback(null,resp.webContentLink,parentUrl)
